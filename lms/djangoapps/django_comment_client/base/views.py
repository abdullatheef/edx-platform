...
from edx_notifications.api.publisher import register_notification_type, publish_notification_to_user
...
def _create_comment(request, course_key, thread_id=None, parent_id=None):
    ...
    if settings.FEATURES["NOTIFICATIONS_ENABLED"] and parent_id is None:
        thread = cc.Thread.find(thread_id)
        action_user_id = int(request.user.id)
        ...
        if not action_user_id == original_poster_id:
            register_publish_notifications(
                ...
                link_to_thread=request.META['HTTP_REFERER']
            )
    if request.is_ajax():
...
def register_publish_notifications(msg_type_name, course_id, original_poster_id, action_user_id,
                                   action_username, thread_title, link_to_thread):
    """
